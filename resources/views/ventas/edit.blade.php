...
<div class="container">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h1 class="h3 m-0">{{ $title }} #{{ $venta->ventas_id }}</h1>
    ...
    <form method="POST" action="{{ route('ventas.update', $venta) }}" id="venta-form">
        @csrf
        @method('PUT')
            ...
            <div class="col-md-4">
                ...
                <select name="cliente_id" class="form-select" required>
                    @foreach($clientes as $cliente)
                        <option value="{{ $cliente->cliente_id }}" @selected($cliente->cliente_id == $venta->cliente_id)>{{ $cliente->nombre }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-4">
                ...
                <select name="empleado_id" class="form-select" required>
                    @foreach($empleados as $empleado)
                        <option value="{{ $empleado->empleados_id }}" @selected($empleado->empleados_id == $venta->empleado_id)>{{ $empleado->nombre }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-4">
                <label class="form-label">Fecha de venta</label>
                <input type="date" name="fecha_venta" class="form-control" value="{{ \Carbon\Carbon::parse($venta->fecha_venta)->format('Y-m-d') }}" required>
            </div>
        </div>
        ...
        <div class="table-responsive">
            <table class="table table-bordered align-middle" id="tabla-productos">
                ...
                <tbody>
                @foreach($venta->detalleVentas as $i => $d)
                    <tr>
                        <td>
                            <select class="form-select producto-select" required>
                                @foreach($productos as $p)
                                    <option value="{{ $p->producto_id }}" data-precio="{{ $p->precio_venta }}" @selected($p->producto_id == $d->producto_id)>{{ $p->nombre }}</option>
                                @endforeach
                            </select>
